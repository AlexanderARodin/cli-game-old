...
impl AltScreen {
    pub fn clean(&mut self) -> Result<()> {
        queue!(self.stdout, Clear(ClearType::All), cursor::Hide,)?;
        Ok(())
    }
...
fn show_prompt(stdout: &mut Stdout) -> Result<()> {
    queue!(stdout, cursor::MoveTo(0, 38), Print(">"), cursor::Show,)?;
    Ok(())
}

fn redraw_background(stdout: &mut Stdout) -> Result<()> {
    queue!(stdout, cursor::MoveTo(2, 0), Print(X_AXIS), Print(Y_AXIS),)?;
    for xx in 0..16 {
        for yy in 0..16 {
...
fn print_on_pos(stdout: &mut Stdout, x: u16, y: u16, s: &str) -> Result<()> {
    let screen_x = x * 4 + 4;
    let screen_y = y * 2 + 2;
    queue!(stdout, cursor::MoveTo(screen_x, screen_y), Print(s),)?;
    Ok(())
}
