...
pub fn main_lua(main_lua_code: &str) -> Result<()> {
    let lua = internal_utils::init_lua(main_lua_code, |txt_list: Vec<String>| {
...
mod tests {
    ...
    fn basic_printer() -> Result<()> {
        ...
        {
            let _ = internal_utils::init_lua(code, |arg_list| {
    ...
    fn ok_loading() -> Result<()> {
        let code = "function update() return {ostacles={},target={x=2,y=3}} end";
        main_lua(code)?;
        Ok(())
    }
    ...
    fn fail_loading() -> Result<()> {
        ...
        assert!(
            ilua.is_err(),
            "Must be a runtime error of abscent UPDATE function"
        );
        Ok(())
    }
}
